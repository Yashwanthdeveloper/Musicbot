      ...
      return message.channel
        .send(
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `Please Give a User ID **\`${prefix}hackban <@userid> <reason>\`**`
            )
      ...
      return message.channel
        .send(
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `Please Give a User ID **User ID not alphabet only Numbers**`
            )
            .setFooter(ee.footertext)
        )
        .then((msg) => msg.delete({ timeout: 5000 }));
        ...
        message.channel
          .send(
            new MessageEmbed()
              .setColor(ee.color)
              .setDescription(
                `<@${user}> SuccesFully Banned From ${message.guild.name}`
              )
              .addField(
                `> Reason :- \`\`${reason != " " ? reason : -"-"}\`\``,
                true
