...
module.exports = {
  ...
  run: async (client, message, args, prefix) => {
    ...
    message
      .reply(
        new MessageEmbed().setDescription(
          `> What is Tittle Of Embed ? || if not then type \`\`'none'\`\``
        )
      )
      ...
      if (title.first().content !== "none") {
        if (title.first().length > 256)
          return message
            .reply(
              new MessageEmbed().setDescription(
                `> Title Can not Biger Than 256 words`
              )
            )
    ...
    message
      .reply(
        new MessageEmbed().setDescription(
          `> What is Description of Embed ? || if not then type \`\`'none'\`\``
        )
      )
      .then((m) => m.delete({ timeout: 30000 }));
      ...
      if (description.first().content !== "none") {
        if (description.first().length > 2048)
          return message
            .reply(
              new MessageEmbed().setDescription(
                `Description Can not Bigger than 2048 Words`
              )
            )
            .then((m) => m.delete({ timeout: 5000 }));
    ...
    message
      .reply(
        new MessageEmbed().setDescription(
          `> What is Colour of Embed ? Please Put Hex Code of Colour || if not then type \`\`'none'\`\``
        )
      )
      .then((m) => m.delete({ timeout: 30000 }));
    ...
    message
      .reply(
        new MessageEmbed().setDescription(
          `> What is Footer of Embed ? || if not then type \`\`'none'\`\``
        )
      )
      .then((m) => m.delete({ timeout: 30000 }));
      ...
      if (footer.first().content !== "none") {
        if (footer.first().length > 100)
          return message
            .reply(
              new MessageEmbed().setDescription(
                `> Footer can not Bigger Than 100 Words`
              )
            )
            .then((m) => m.delete({ timeout: 5000 }));
